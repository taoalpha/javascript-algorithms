...
describe("DoublyLinkedList", () => {
  ...
  it("should find node by callback", () => {
    const linkedList: DoublyLinkedList<{value: number, key: string}> = new DoublyLinkedList();
    ...
    const node = linkedList.find({ criteria: value => value.key === "test2" });

    expect(node).toBeDefined();
    expect(node.value.value).toBe(2);
    expect(node.value.key).toBe("test2");
    expect(linkedList.find({ criteria: value => value.key === "test5" })).toBeNull();
  });
  ...
  it("should find node by means of custom compare function", () => {
    ...
    const linkedList: DoublyLinkedList<{value: number, customValue: string}> = new DoublyLinkedList(comparatorFunction);
