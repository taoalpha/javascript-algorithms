 ...
 * @return {boolean}
 */
function isSafe(adjacencyMatrix, verticesIndices: MixMap<number, number>, cycle: GraphVertex<number>[], vertexCandidate: GraphVertex<number>) {
  const endVertex = cycle[cycle.length - 1];

  // Get end and candidate vertices indices in adjacency matrix.
  const candidateVertexAdjacencyIndex = verticesIndices.get(vertexCandidate.getKey());
  const endVertexAdjacencyIndex = verticesIndices.get(endVertex.getKey());
 ...
 * @return {boolean}
 */
function isCycle(adjacencyMatrix, verticesIndices: MixMap<number, number>, cycle) {
  ...
  const startVertexAdjacencyIndex = verticesIndices.get(startVertex.getKey());
  const endVertexAdjacencyIndex = verticesIndices.get(endVertex.getKey());
 ...
 * @return {GraphVertex[][]}
 */
export default function hamiltonianCycle(graph: Graph<number>) {
