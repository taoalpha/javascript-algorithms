 ...
 * @return {Object}
 */
export default function graphBridges(graph: Graph<string>) {
  ...
  const dfsCallbacks = {
     ...
     */
    leaveVertex: ({ currentVertex, previousVertex } : {[key: string] : GraphVertex<string>}) => {
      ...
      if (parentDiscoveryTime < currentLowDiscoveryTime) {
        ...
        bridges[bridge.getKey().toString()] = bridge;
      }
    },
