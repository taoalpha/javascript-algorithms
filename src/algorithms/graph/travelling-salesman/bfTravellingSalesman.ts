 ...
 * @return {number}
 */
function getCycleWeight(adjacencyMatrix, verticesIndices: MixMap<number, number>, cycle) {
  let weight = 0;
  ...
  for (let cycleIndex = 1; cycleIndex < cycle.length; cycleIndex += 1) {
    ...
    const fromVertexIndex = verticesIndices.get(fromVertex.getKey());
    const toVertexIndex = verticesIndices.get(toVertex.getKey());
